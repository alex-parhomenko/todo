...
class Logger(logging.getLoggerClass()):
    ...
    # acceptable format for its value like: {name} | {levelname} | {asctime} | {message}
    ...
    def __get_valid_config_field_value(self, conf_field_name):
        ...
        '''
        val = self.__config[conf_field_name] if conf_field_name in self.__config else None
        if val is None:
            # Default value
            val = self.__def_config[conf_field_name] if conf_field_name in self.__def_config else None
        if val is None:
            raise LoggerException('Config field:', conf_field_name,
                                  ' Value is not found')
        return val
    ...
    def __get_file_hadler(self, confValues, output_dir):
        ...
        full_fname = os.path.join(
            output_dir, confValues[Logger.CONFIG_FIELD_FILE_NAME])
